...
QueryData genHostOpenSockets(QueryContext& context) {
  Status status;
  QueryData results;
  ...
  if (!pid_filter) {
    pids.clear();
    status = osquery::procHostProcesses(pids);
    if (!status.ok()) {
  ...
  SocketInodeToHostProcessInfoMap inode_hostproc_map;
  HostSocketInfoList hostsocket_list;
  for (const auto& pid : pids) {
    /* Step 1 */
    status = procGetSocketInodeToHostProcessInfoMap(pid, inode_hostproc_map);
    if (!status.ok()) {
    ...
    status = procGetHostProcessNamespaces(pid, namespaces, {"net"});
    if (status.ok()) {
      if (namespaces.empty()) {
        ns = 0;
      } else {
    ...
    if (netns_list.count(ns) == 0) {
      ...
      for (const auto& pair : kLinuxProtocolNames) {
        status = procGetHostSocketList(AF_INET, pair.first, ns, pid, hostsocket_list);
        if (!status.ok()) {
          VLOG(1)
        ...
        status = procGetHostSocketList(AF_INET6, pair.first, ns, pid, hostsocket_list);
        if (!status.ok()) {
          VLOG(1)
              << "Results for process_open_sockets might be incomplete. Failed "
      ...
      status = procGetHostSocketList(AF_UNIX, IPPROTO_IP, ns, pid, hostsocket_list);
      if (!status.ok()) {
        VLOG(1)
            << "Results for process_open_sockets might be incomplete. Failed "
   ...
   */
  for (const auto& info : hostsocket_list) {
    Row r;
    auto proc_it = inode_hostproc_map.find(info.socket);
    if (proc_it != inode_hostproc_map.end()) {
